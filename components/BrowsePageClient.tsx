...
export default function BrowsePageClientWrapper() {
  return (
    <Suspense fallback={<div className="text-center py-16 text-gray-500">Loading...</div>}>
      <BrowsePageInner stories={[]} categories={[]} tags={[]} />
    </Suspense>
  );
}

export function BrowsePageInner({ stories: initialStories, categories: initialCategories, tags: initialTags }: BrowsePageClientProps) {
  ...
  useEffect(() => { if (genreParam) setActiveCategory(genreParam); }, [genreParam]); // eslint-disable-line react-hooks/set-state-in-effect
  useEffect(() => { setCurrentPage(1); }, [activeCategory, searchQuery, sortBy, perPage, selectedTags]); // eslint-disable-line react-hooks/set-state-in-effect
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
            <div className="flex-1">
              <select
                ...
                className="md:hidden w-full sm:w-auto px-3 py-2 text-xs rounded-lg bg-white text-gray-700 border border-gray-300 focus:outline-none focus:border-accent dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 [&>option]:bg-white [&>option]:text-gray-700 dark:[&>option]:bg-gray-800 dark:[&>option]:text-gray-300"
              >
          ...
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden border border-gray-200 dark:border-gray-700" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(80vh-80px)]">
              {selectedTags.length > 0 && (
                <div className="mb-4 pb-4 border-b border-gray-200 dark:border-gray-700">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={clearAllTags} className="text-xs text-accent hover:underline">Clear all</button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {selectedTags.map(tag => (
                      <button key={tag} onClick={() => toggleTag(tag)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium bg-accent text-white hover:opacity-90 transition-opacity">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {allTags.map(tag => (
                  <button key={tag} onClick={() => toggleTag(tag)} className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors text-left ${selectedTags.includes(tag) ? 'bg-accent/20 text-accent border-2 border-accent' : 'bg-gray-50 text-gray-700 hover:bg-gray-100 border-2 border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 dark:border-gray-700'}`}>
                    {tag}
                  </button>
                ))}
              </div>
            ...
            <div className="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowTagModal(false)} className="px-4 py-2 rounded-full bg-accent text-white text-sm font-medium hover:opacity-90 transition-opacity">Done</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => setShowGenreModal(false)}>
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden border border-gray-200 dark:border-gray-700" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 overflow-y-auto max-h-[calc(80vh-80px)]">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                <button
                  ...
                  className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeCategory === 'All' || activeCategory === translations.id.home.all || activeCategory === translations.en.home.all ? 'bg-accent text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100 border-2 border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 dark:border-gray-700'}`}
                >
                  {translations[lang].home.all}
                </button>
                {categories.map(cat => (
                  <button key={cat} onClick={() => { setActiveCategory(cat); setShowGenreModal(false); }} className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeCategory === cat ? 'bg-accent text-white' : 'bg-gray-50 text-gray-700 hover:bg-gray-100 border-2 border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700 dark:border-gray-700'}`}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            <div className="sticky bottom-0 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center justify-between">
              ...
              <button onClick={() => setShowGenreModal(false)} className="px-4 py-2 rounded-full bg-accent text-white text-sm font-medium hover:opacity-90 transition-opacity">Done</button>
            </div>
          </div>
        </div>
      )}
